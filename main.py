...
file = '/Users/akshay/Codes/Python/Cisco/conf_2039.rtf'
content = []
...
def sevCheck(num):
    global oneWarnings, twoWarnings, threeWarnings
    if num == 1:
        oneWarnings += 1
    elif num == 2:
        twoWarnings += 1
...
if (encryption.enc.check_weak_encryption(file)):
    ...
    print(encryption.enc.check_weak_encryption(file))
    content.append("\n")
...
print(f"Device name: {hostname(file)}")
print(datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-7])
print(f"""Number of Severity 1 findings: {oneWarnings}
Number of Severity 2 findings: {twoWarnings}
Number of Severity 3 findings: {threeWarnings}""")
print("\n\n")
for x in content:
    print(x)
print("\n")
